...
| **Criteria: Traditional Fit** | `ops-wiki/criteria/trad_score.md` | Career alignment rubric |
| **Criteria: Agency Score** | `ops-wiki/criteria/agency_score.md` | Stackability & automation rubric |
| **Criteria: Leverage Score** | `ops-wiki/criteria/leverage_score.md` | Strategic career leverage rubric |
| **Criteria: Employer Score** | `ops-wiki/criteria/employer_score.md` | Employer sanity & job integrity rubric |
| **Niches** | `ops-wiki/niches/*.md` | Niche definitions, search strategies, niche-specific scoring criteria |
| **Claims** | `xp-wiki/claims/*.md` | Verified skill & technology registry |
| **Experience** | `xp-wiki/experience/*.md` | Curated role history |
| **Accomplishments** | `xp-wiki/accomplishments/*.md` | STAR case studies |
| **Evidence** | `xp-wiki/evidence/*.md` | Raw CSV/spreadsheet citation sources |
...
| **Applications** | `ops-wiki/applications/*.md` | Promoted jobs with full evaluations and tailored resumes |
| **Resumes** | `ops-wiki/job-docs/resumes/` | Resume drafts |
| **Templates** | `ops-wiki/job-docs/resume-templates/` | Niche templates |
| **Strategy** | `ops-wiki/strategy/` | Search experiment logs and niche refinement notes |
...
**Trigger:** Any job in `ops-wiki/jobs/` whose frontmatter contains `promoted: true` AND does NOT contain `resume_drafted: true`.

**Procedure:**
1. Read the job's full listing file.
2. Read the relevant niche file from `ops-wiki/niches/` (matched via the query label alias).
3. If the niche has a `template_resume` property, read that template from `ops-wiki/job-docs/resume-templates/`.
4. Match the job's required skills against `xp-wiki/claims/*.md` to retrieve pre-verified evidence.
5. Match the job's required accomplishments against `xp-wiki/accomplishments/*.md` for STAR-formatted case studies.
...
7. Save the resume draft to `ops-wiki/job-docs/resumes/<date>_<company>_<title>.md`.
8. Update the job listing's frontmatter: `resume_drafted: true`, `resume_file: "[[ops-wiki/job-docs/resumes/resume-filename]]"`.
9. Create or update the application file in `ops-wiki/applications/`.
10. Append to `next_report.md`: job title, company, scores, resume file link.

### Priority 2: Deep Evaluation of Promoted Jobs

**Trigger:** Any job in `ops-wiki/jobs/` whose frontmatter contains `promoted: true` AND does NOT contain `deep_eval: true`.
...
**Trigger:** Search reports in `ops-wiki/searches/` containing job entries in their `results` array that lack a `first_pass` score block.
...
**Procedure:**
1. Read the most recent unscored search reports in `ops-wiki/searches/`.
...
6. **Promotion Decision:** If `overall ≥ 75` AND no critical red flags, the job is promoted.
   - Open the *individual job listing file* in `ops-wiki/jobs/<label>/`.
...
**Trigger:** A niche in `ops-wiki/niches/` that has not been queried recently.
...
3. Call the job-ferret API:
   ...
   ```
4. Verify the report generated in `ops-wiki/searches/`.
...
**Procedure:**
1. Identify the query by reviewing past reports in `ops-wiki/searches/`.
...
2. Document findings in `ops-wiki/strategy/`.
...
**Procedure:**
1. Create/update page in `ops-wiki/employers/<company-name>.md`.
...
1. Scan `ops-wiki/searches/` for any report missing `first_pass` evaluations in its `results` array. (Add to Priority 3).
2. Scan individual files in `ops-wiki/jobs/*/` with `promoted: true` but missing `deep_eval`. (Add to Priority 2).
